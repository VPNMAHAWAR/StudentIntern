...
<head>
  ...
  <div class="container text-center">
    ...
    <?php
      if($type == 'E'){
        echo '<h1 class="text-left px-5">This is a Employer account <br>Please Sign Out and Sign In as Student to Apply</h1>';
      }
      else{
        $q0 = "SELECT * FROM interns WHERE sid='$id';";
        $r0 = mysqli_query($dbc, $q0);
        if($r0){
          $row = mysqli_fetch_array($r0);
          if ($row[$iid] == 'T'){
            echo 'Already Applied';
          }
          else{
            $q1 = "UPDATE interns SET `$iid`='T' WHERE sid='$id';";
            $r1 = mysqli_query($dbc, $q1);
            
            if($r1){
                echo '<img src="img/ok.png" alt="Internship Applied Successfully!!!" style="max-width:80%;float: center;">';
